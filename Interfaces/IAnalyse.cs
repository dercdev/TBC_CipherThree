...
namespace Interfaces
{
    public interface IAnalyse
    {
        ...
        List<Characteristic> FindSpecifiedDifferentialDepthSearch(int inputDiff, int outputDiff, int round, List<Differential> differentialNumList);
        List<Characteristic> FindBestCharacteristicsDepthSearch(DifferentialAttackRoundConfiguration roundConfiguration, List<Differential> differentialsList);
        List<Differential> CountDifferentialsSingleSBox();
        List<Pair> FilterPairs(DifferentialAttackRoundConfiguration roundConfig, List<Differential> diffListOfSBox, DifferentialKeyRecoveryAttack attack, IEncryption encryption, int expectedDifferential);
        ...
        DifferentialAttackRoundConfiguration GenerateConfigurationAttack(int round, bool[] sBoxesToAttack, AbortingPolicy abortingPolicy, SearchPolicy searchPolicy ,List<Differential> diffListOfSBox, DifferentialKeyRecoveryAttack attack, IEncryption encryption);
        DifferentialAttackLastRoundResult AttackFirstRound(DifferentialKeyRecoveryAttack attack, IEncryption encryption);
        DifferentialAttackRoundConfiguration RefreshPairLists(DifferentialAttackRoundConfiguration roundConfig, List<Differential> diffListOfSBox, DifferentialKeyRecoveryAttack attack, IEncryption encryption);
